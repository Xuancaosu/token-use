...
///   且不一定等于用户目录，可能导致 `.token-use/token-use.db` 路径变化，从而“看起来像数据丢失”。
///
/// ## 测试隔离
///
...
/// 获取应用配置目录路径 (~/.token-use)
pub fn get_app_config_dir() -> PathBuf {
    if let Some(custom) = crate::app_store::get_app_config_dir_override() {
        return custom;
    }

    let default_dir = get_home_dir().join(".token-use");

    // 兼容 v3.10.3：当用户环境存在 `HOME` 且与真实用户目录不同，
    // v3.10.3 可能在 `HOME/.token-use/` 下创建/使用了数据库。
    // 这里仅在“默认位置没有数据库”时回退到旧位置，避免再次出现“供应商消失”问题，
    // 同时也避免新安装因为 `HOME` 被设置而写入非预期路径。
    #[cfg(windows)]
    {
        let default_db = default_dir.join("token-use.db");
        if !default_db.exists() {
            if let Ok(home_env) = std::env::var("HOME") {
                let trimmed = home_env.trim();
                if !trimmed.is_empty() {
                    let legacy_dir = PathBuf::from(trimmed).join(".token-use");
                    if legacy_dir.join("token-use.db").exists() {
                        log::info!(
                            "Detected legacy Token Use database at {}, using it instead of {}",
                            legacy_dir.display(),
                            default_dir.display()
                        );
                        return legacy_dir;
...
mod tests {
    ...
    #[test]
    fn app_config_dir_defaults_to_token_monitor_home() {
        let dir = get_app_config_dir();
        assert!(dir.ends_with(".token-use"));
    }

    #[test]
    fn app_config_path_uses_token_monitor_dir() {
        let path = get_app_config_path();
        assert!(path.ends_with(PathBuf::from(".token-use").join("config.json")));
    }
